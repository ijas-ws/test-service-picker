...
export function BackendNodeHapiPg({ stack }) {
	const clientName = "backend";
	const environment = "develop";
	const dbName = "backend_database_develop";
	...
	// ECS Security groups
	const ecsSG = new ec2.SecurityGroup(stack, `${clientName}-ecsSG`, {
		vpc,
		allowAllOutbound: true,
	});
	...
	const taskRole = new iam.Role(
		stack,
		`${clientName}-task-role-${environment}`,
		{
	...
	const image = ecs.ContainerImage.fromAsset("backend-node-hapi-pg/", {
	...
	const container = taskDefinition.addContainer(`${clientName}-container-${environment}`, {
		image,
		memoryLimitMiB: 512,
		environment: {
			BUILD_NAME: "develop",
